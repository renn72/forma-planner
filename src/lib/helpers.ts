...
const generateAttempts = (targetWeight: number): number[] => {
  if (!targetWeight || targetWeight <= 0) {
    return Array(9).fill(0)
  }

  return [
    roundLift(targetWeight, 0.9) === roundLift(targetWeight, 0.91) 
      ? roundLift(targetWeight, 0.9) - 2.5 
      : roundLift(targetWeight, 0.9),
    roundLift(targetWeight, 0.91),
    roundLift(targetWeight, 0.92) === roundLift(targetWeight, 0.91) 
      ? roundLift(targetWeight, 0.92) + 2.5 
      : roundLift(targetWeight, 0.92),
    roundLift(targetWeight, 0.95) === roundLift(targetWeight, 0.96) 
      ? roundLift(targetWeight, 0.95) - 2.5 
      : roundLift(targetWeight, 0.95),
    roundLift(targetWeight, 0.96),
    roundLift(targetWeight, 0.97) === roundLift(targetWeight, 0.96) 
      ? roundLift(targetWeight, 0.97) + 2.5 
      : roundLift(targetWeight, 0.97),
    roundLift(targetWeight, 0.99) === roundLift(targetWeight, 1) 
      ? roundLift(targetWeight, 0.99) - 2.5 
      : roundLift(targetWeight, 0.99),
    roundLift(targetWeight, 1),
    roundLift(targetWeight, 1.02) === roundLift(targetWeight, 1) 
      ? roundLift(targetWeight, 1.02) + 2.5 
      : roundLift(targetWeight, 1.02),
  ]
}
...
const generateWarmups = (opener: number): { weight: number; reps: string }[] => {
  if (!opener || opener <= 0) {
    return []
  }

  const tableTarget = warmupTable.reduce((acc, curr, idx, array) => {
    if (array[idx + 1]) {
    ...
    return acc
  }, warmupTable[warmupTable.length - 1])

  const weights = tableTarget?.slice(1) || []
  const reps: string[] = []
  ...
  return weights.map((w, i) => ({ weight: w, reps: reps[i] || '1' }))
}
...
export const calculateOldWilks = (
  bodyWeight: number,
  weightLifted: number,
  isFemale: boolean,
) => {
...
export { generateAttempts, generateWarmups }
